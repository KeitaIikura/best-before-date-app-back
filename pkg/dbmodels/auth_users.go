...
var AuthUserTableColumns = struct {
	...
	EmailAddress: "auth_users.email_address",
...
var AuthUserWhere = struct {
	...
	ID:           whereHelperint64{field: "`auth_users`.`id`"},
	Name:         whereHelperstring{field: "`auth_users`.`name`"},
	EmailAddress: whereHelperstring{field: "`auth_users`.`email_address`"},
	Password:     whereHelperstring{field: "`auth_users`.`password`"},
...
var (
	authUserAllColumns            = []string{"id", "name", "email_address", "password", "created_at", "updated_at"}
	authUserColumnsWithoutDefault = []string{"name", "email_address", "password"}
	authUserColumnsWithDefault    = []string{"id", "created_at", "updated_at"}
...
func (q authUserQuery) One(ctx context.Context, exec boil.ContextExecutor) (*AuthUser, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "dbmodels: failed to execute a one query for auth_users")
	}
...
func (q authUserQuery) All(ctx context.Context, exec boil.ContextExecutor) (AuthUserSlice, error) {
	...
	err := q.Bind(ctx, exec, &o)
	if err != nil {
		return nil, errors.Wrap(err, "dbmodels: failed to assign all query results to AuthUser slice")
	}
...
func (q authUserQuery) Count(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "dbmodels: failed to count auth_users rows")
	}
...
func (q authUserQuery) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	...
	err := q.Query.QueryRowContext(ctx, exec).Scan(&count)
	if err != nil {
		return false, errors.Wrap(err, "dbmodels: failed to check if auth_users exists")
	}
...
func FindAuthUser(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*AuthUser, error) {
	...
	query := fmt.Sprintf(
		"select %s from `auth_users` where `id`=?", sel,
	)
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "dbmodels: unable to select from auth_users")
	}
...
func (o *AuthUser) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	if o == nil {
		return errors.New("dbmodels: no auth_users provided for insertion")
	}
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `auth_users` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `auth_users` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, authUserPrimaryKeyColumns))
		}
	...
	if err != nil {
		return errors.Wrap(err, "dbmodels: unable to insert into auth_users")
	}
	...
	if err != nil {
		return errors.Wrap(err, "dbmodels: unable to populate default values for auth_users")
	}
...
func (o *AuthUser) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		if len(wl) == 0 {
			return 0, errors.New("dbmodels: unable to update auth_users, could not build whitelist")
		}
	...
	if err != nil {
		return 0, errors.Wrap(err, "dbmodels: unable to update auth_users row")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "dbmodels: failed to get rows affected by update for auth_users")
	}
...
func (q authUserQuery) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "dbmodels: unable to update all for auth_users")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "dbmodels: unable to retrieve rows affected for auth_users")
	}
...
func (o AuthUserSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE `auth_users` SET %s WHERE %s",
		...
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, authUserPrimaryKeyColumns, len(o)))
	...
	if err != nil {
		return 0, errors.Wrap(err, "dbmodels: unable to update all in authUser slice")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "dbmodels: unable to retrieve rows affected all in update all authUser")
	}
	return rowsAff, nil
}

var mySQLAuthUserUniqueColumns = []string{
	"id",
	"email_address",
}
...
func (o *AuthUser) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
		return errors.New("dbmodels: no auth_users provided for upsert")
	}
	if !boil.TimestampsAreSkipped(ctx) {
		currTime := time.Now().In(boil.GetLocation())
	...
	nzDefaults := queries.NonZeroDefaultSet(authUserColumnsWithDefault, o)
	nzUniques := queries.NonZeroDefaultSet(mySQLAuthUserUniqueColumns, o)
	...
	if !cached {
		...
		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("dbmodels: unable to upsert auth_users, could not build update column list")
		}

		ret = strmangle.SetComplement(ret, nzUniques)
		cache.query = buildUpsertQueryMySQL(dialect, "`auth_users`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `auth_users` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "dbmodels: unable to upsert for auth_users")
	}
	...
	uniqueMap, err = queries.BindMapping(authUserType, authUserMapping, nzUniques)
	if err != nil {
		return errors.Wrap(err, "dbmodels: unable to retrieve unique values for auth_users")
	}
...
func (o *AuthUser) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	if o == nil {
		return 0, errors.New("dbmodels: no AuthUser provided for delete")
	}
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "dbmodels: unable to delete from auth_users")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "dbmodels: failed to get rows affected by delete for auth_users")
	}
...
func (q authUserQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	if q.Query == nil {
		return 0, errors.New("dbmodels: no authUserQuery provided for delete all")
	}
	...
	result, err := q.Query.ExecContext(ctx, exec)
	if err != nil {
		return 0, errors.Wrap(err, "dbmodels: unable to delete all from auth_users")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "dbmodels: failed to get rows affected by deleteall for auth_users")
	}
...
func (o AuthUserSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "dbmodels: unable to delete all from authUser slice")
	}
...
func (o *AuthUserSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	if err != nil {
		return errors.Wrap(err, "dbmodels: unable to reload all in AuthUserSlice")
	}
...
func AuthUserExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `auth_users` where `id`=? limit 1)"
	...
	err := row.Scan(&exists)
	if err != nil {
		return false, errors.Wrap(err, "dbmodels: unable to check if auth_users exists")
	}
